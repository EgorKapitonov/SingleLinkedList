...
public:
    ...
    //Объект, у которого есть итераторы begin и end
    ...
    SingleLinkedList(std::initializer_list<Type> values) {
        ...
        list.In(values.begin(), values.end());
        swap(list);
    }

    SingleLinkedList(const SingleLinkedList& other) {
        SingleLinkedList list;
        list.In(other.begin(), other.end());
        swap(list);
    }
    ...
    */
    ...
    Iterator EraseAfter(ConstIterator pos) noexcept {
        assert(!IsEmpty());
        assert(pos.node_);
        Node* new_node = pos.node_->next_node->next_node;
    ...
    void Clear() noexcept {
        ...
        while (head_.next_node != nullptr) {
            //Голова списка ссылается на его второй элемент 
            Node* ptr_ = head_.next_node->next_node;
            //Первый элемент удаляется
            delete head_.next_node;
            //Голова списка присваивает значение второго элемента
            head_.next_node = ptr_;
        }
        //Обнуление размер списка
        size_ = 0;
    }
